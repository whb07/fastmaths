...
fn bench_erf(c: &mut Criterion) {
    ...
    bench_inputs(&mut group, &inputs, fastmaths::erf, glibc_erf);
    group.finish();

    let mut group = c.benchmark_group("erf/common");
    bench_inputs(&mut group, &common, fastmaths::erf, glibc_erf);
    group.finish();

    let mut group = c.benchmark_group("erf/wide");
    bench_inputs(&mut group, &wide, fastmaths::erf, glibc_erf);
    group.finish();
}
