...
fn bench_acosh(c: &mut Criterion) {
    ...
    bench_inputs(&mut group, &inputs, fastmaths::acosh, glibc_acosh);
    group.finish();

    let mut group = c.benchmark_group("acosh/common");
    bench_inputs(&mut group, &common, fastmaths::acosh, glibc_acosh);
    group.finish();

    let mut group = c.benchmark_group("acosh/wide");
    bench_inputs(&mut group, &wide, fastmaths::acosh, glibc_acosh);
    group.finish();
}
