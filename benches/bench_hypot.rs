...
fn bench_hypot(c: &mut Criterion) {
    ...
    bench_inputs2(&mut group, &inputs, fastmaths::hypot, glibc_hypot);
    group.finish();

    let mut group = c.benchmark_group("hypot/wide");
    bench_inputs2(&mut group, &wide, fastmaths::hypot, glibc_hypot);
    group.finish();

    let mut group = c.benchmark_group("hypot/huge");
    bench_inputs2(&mut group, &huge, fastmaths::hypot, glibc_hypot);
    group.finish();
}
