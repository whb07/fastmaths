...
#[inline(always)]
fn f64_from_bits(u: u64) -> f64 { f64::from_bits(u) }
#[inline(always)]
fn f64_to_bits(x: f64) -> u64 { x.to_bits() }

#[inline(always)]
fn hi_word(x: f64) -> u32 { (f64_to_bits(x) >> 32) as u32 }
#[inline(always)]
fn lo_word(x: f64) -> u32 { (f64_to_bits(x) & 0xffff_ffffu64) as u32 }
#[inline(always)]
...
#[inline(always)]
fn get_exp_bits(u: u64) -> i32 { ((u >> 52) & 0x7ff) as i32 }
...
fn scalbn(mut x: f64, n: i32) -> f64 {
    ...
    if e == 0 {
        ...
        if ne <= 0 {
            // underflow/subnormal
            return x * f64_from_bits(((ne + 1023 + 54) as u64) << 52) * f64_from_bits(0x3c90_0000_0000_0000u64);
        }
